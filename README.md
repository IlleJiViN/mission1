...
## 📈 12. 대규모 데이터 확장 시 성능 및 메모리 분석 (1,000개 이상 데이터 세트)

본 시스템이 1,000개 이상의 퀴즈 데이터로 확장될 때 발생하는 주요 병목 현상과 이를 해결하기 위한 엔지니어링 개선 방안은 다음과 같습니다.

### 1) 메모리 부하 및 I/O 병목 분석
- **단일 JSON 파일 구조의 한계**: 전체 데이터셋(1,000개 이상)을 `state.json` 단일 파일로 동기 읽기/쓰기를 수행할 경우, 데이터 조회 시마다 파일 전체를 메모리(RAM)로 파싱하여 로드하므로 메모리 사용량이 급증합니다.
- **파일 직렬화/역직렬화 오버헤드**: 파이썬 `json.load()` / `json.dump()` 사용 시 전체 텍스트를 인코딩/디코딩하는 $O(M)$ ($M$: 전체 데이터 크기) 파일 I/O 지연이 발생합니다.

### 2) 성능 최적화 대책 (인덱싱 및 페이징)
- **페이징 (Paging / Chunking)**:
  - 콘솔 UI 또는 퀴즈 목록 조회 시 1,000개의 항목을 한 번에 출력하지 않고, **페이지 단위 (예: Page당 10~20개 항목)** 로 슬라이싱하여 조회합니다.
  - 데이터를 필요한 범위만 메모리에 유지함으로써 메모리 발자국(Memory Footprint)을 최소화하고 응답 지연을 방지합니다.
- **인덱싱 및 RDBMS/NoSQL 파일 분할**:
  - JSON 파일 방식을 **SQLite** 또는 **Key-Value DB(RocksDB, LevelDB)** 구조로 전환합니다.
  - 데이터의 고유 ID, 카테고리, 난이도 등에 **B-Tree 인덱스**를 구축하여 전체 탐색($O(N)$) 대신 **인덱스 탐색($O(\log N)$)** 으로 원하는 항목을 최적의 시간에 조회합니다.
- **Generator / Streaming 파싱**:
  - 파이썬의 `ijson` 라이브러리와 같은 스트리밍 파서를 사용하여 1,000개 이상의 JSON 객체를 메모리에 한꺼번에 올리지 않고 차례로 제너레이터(Generator) 처리합니다.

---

## 🚀 13. 향후 개선 사항 (Future Improvements)
